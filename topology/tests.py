...
class TopologyViewSetTest(ParentTest):
  def setUp(self):
    super().setUp()
    self.sub_a = Substation.objects.create(zone=self.zone_a, name = 'Substation Alpha', max_capa_mw=100)
    self.feeder_a = Feeder.objects.create(substation=self.sub_a, code = 'FDR-A01', is_energized=True)
    self.trans_a = Transformer.objects.create(feeder=self.feeder_a, uid = 'TR-001', kva_rating=500)
    self.branch_a = Branch.objects.create(transformer=self.trans_a, account_number = 'ACC-101', type = 'industrial')

  def test_feeder_list_isola(self):
    self.auth(self.engineer_a)
    url = reverse('feeder-list')
    resp = self.client.get(url)
    self.assertEqual(resp.status_code, 200)
    self.assertEqual(len(resp.data), 1)
    self.assertEqual(resp.data[0]['code'], 'FDR-A01')

  def test_feeder_toggle_perm(self):
    self.auth(self.officer_b)
    url = reverse('feeder-toggle', kwargs={'pk': self.feeder_a.pk})
    resp = self.client.patch(url)
    self.assertEqual(resp.status_code, 404)

  def test_grid_tree_retriev(self):
    self.auth(self.engineer_a)
    url = reverse('grid-tree')
    resp = self.client.get(url)
    self.assertEqual(resp.status_code, 200)
    self.assertEqual(resp.data['grid_name'], 'zone 1')
    self.assertTrue(len(resp.data['substations']) > 0)