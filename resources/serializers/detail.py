...
class GenerationUnitSerializer(serializers.ModelSerializer):
  ...
  records = serializers.StringRelatedField(many=True, read_only=True)
  class Meta:
    model = GenerationUnit
    fields = ['source_name', 'source', 'fuel_type', 'fuel_name', 'unit_name', 'records', 'installed_capacity_mw', 'curr_output_mw', 'cost_per_unit', 'operational']
